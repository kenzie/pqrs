...
denominator_validation :patient_age do |answers|
  next {:pass => true} if (18..75).cover? answers[:patient_age]
  {:pass => false, :reason => "Patient was not between 18 and 75 years of age during the year of encounter."}
end
...
numerator_validation do |answers|
  if answers[:q1] && answers[:q1] > 9.0
    ["1234F", :pass]
  elsif answers[:q1] && answers[:q1].between?(7.0, 9.0)
    ["1234F-2P", :exclude]
  elsif answers[:q1] && answers[:q1] < 7.0
    ["1234F-3P", :exclude]
  elsif answers[:q2]
    ["1234F-8P", :fail]
  end
